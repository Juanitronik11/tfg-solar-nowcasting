# -*- coding: utf-8 -*-
# Run:  caffeinate -d -i python src/def_train_future_ghiphase_m1.py
# FUTURE — Phase 2 (GHI-focused ~4h). Starts from the best segmentation and keeps SEG stable.
...
ALPHA               = 0.5
OUTPUT_STRIDE       = 16
LR_TARGET           = 7.5e-5      # slightly lower than in SEG
LR_WARMUP_EPOCHS    = 3
CLIPNORM            = 1.0

# Weights in the GHI phase (maintain SEG without degrading it)
LAMBDA_SEG          = 0.7         # SEG still counts
GHI_WEIGHT          = 1.8         # GHI carries more weight
LAMBDA_DICE         = 1.0
...
# ---- FUTURE model — compiled for GHI-focused training while maintaining SEG ----
def build_future_compiled_ghi(img_h: int, img_w: int, num_classes: int, timesteps: int,
    ...
    # Ensure attention and GHI head are enabled (trainable=True)
    n1 = set_trainable_by_name(fut, ("attn", "attention"), True)
    n2 = set_trainable_by_name(fut, ("ghi_pred_inner", "ghi_head"), True)
    print(f"[unfreeze] atención trainable+= {n1},  cabeza GHI trainable+= {n2}")
...
# in def_train_future_ghiphase_m1.py
def try_resume_future_with_priority(outdir: Path, model: tf.keras.Model) -> bool:
    patterns = [
        "*ghiphase_best.index",           # 1) best GHI of this phase (if retraining)
        "*segphase_best_segloss.index",   # 2) ← start from the best SEG
        "*_best.index",                   # 3) best generic GHI (if there was no SEG)
        "*_best_segloss.index",           # 4) another best of SEG, just in case
        "*_last.index",                   # 5) last
    ]
    for pat in patterns:
...
def make_callbacks(tag: str, outdir: Path):
    ...
    # Soft limiter: if SEG loss worsens too much, reduce LR
    ...
    return cbs, ckpt_best_ghi, ckpt_best_seg, ckpt_last
...
def main():
    ...
    # Load best from SEG phase
    loaded = try_resume_future_with_priority(OUTDIR, fut)

    tag = f"future{_RNN.upper()}_T{TIMESTEPS}_s{STRIDE}_{dl.IMG_SIZE[0]}x{dl.IMG_SIZE[1]}_a{ALPHA}_os{OUTPUT_STRIDE}_b{BATCH}_seed{SEED}_ghiphase"
    callbacks, ck_best_ghi, ck_best_seg, ck_last = make_callbacks(tag=tag, outdir=OUTDIR)

    print(f"\n>>> FUTURE-GHI({_RNN.upper()}) IMG={dl.IMG_SIZE}, T={TIMESTEPS}, stride={STRIDE}, "
    ...
    try:
        ...
        callbacks = [tf.keras.callbacks.LearningRateScheduler(
                        lambda e, lr: LR_TARGET*(0.25 + 0.75*min(1.0, (e+1)/max(1,LR_WARMUP_EPOCHS))), verbose=0)
                     ] + callbacks + [TimeLimit(MAX_MINUTES)]
        ...
        meta = {
            ...
            "checkpoints": {"best_ghi": str(ck_best_ghi), "best_segloss": str(ck_best_seg), "last": str(ck_last)},
