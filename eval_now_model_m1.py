...
# - NEW: --replot-only to rebuild figures from cached summary/NPY without re-evaluating models.
# - NEW: --skip-eval-if-exists to avoid re-evaluating when a report already exists; figures are regenerated from cache.
#
# Examples:
...
def set_policy_float32():
    """Force float32 policy (disable mixed precision) to avoid numerical diffs during eval."""
    from tensorflow.keras import mixed_precision
    mixed_precision.set_global_policy("float32")

def _count_csv_rows(csv_path: Path) -> int:
    """Quickly count rows (minus header) to estimate number of batches for nicer progress bars."""
    try:
...
def ensure_targets_SI(ds: tf.data.Dataset) -> tf.data.Dataset:
    """If the dataset loader outputs normalized targets (e.g., GHI), convert them back to SI units for reporting."""
...
def build_now_for_eval() -> tf.keras.Model:
    """Build the NOW model (segmentation head included) for evaluation only."""
...
def make_now_dataset(csv_path: Path, batch: int):
    """Create the NOW dataset for evaluation. Falls back to FUTURE dataset (T=1) if needed."""
...
def make_seg_metrics():
    """Build the metrics dictionary for segmentation evaluation."""
    return dict(
...
def iou_inter_union(y_true, y_pred_probs):
    """Compute total intersection and union per class across the whole dataset (with mask)."""
...
def confusion_accumulators(num_classes: int):
    """Allocate confusion matrix (raw counts)."""
    return np.zeros((num_classes, num_classes), dtype=np.int64)

def update_confmat(conf, y_true, y_pred_probs):
    """Accumulate confusion counts ignoring invalid pixels per valid mask."""
...
# --------- Class names and confusion plotting ----------
def class_names_all(include_void=True):
    """Build a class-name list indexed from 0..NUM_CLASSES-1 using model constants; falls back to C{i}."""
    mapping = {
        int(models.SKY_IDX): "SKY",
        int(models.THICK_IDX): "THICK",
        int(models.THIN_IDX): "THIN",
        int(models.SUN_IDX): "SUN",
    }
    if include_void and hasattr(models, "VOID_IDX"):
        mapping[int(models.VOID_IDX)] = "VOID"
    return [mapping.get(i, f"C{i}") for i in range(NUM_CLASSES)]

def plot_confusion(cm: np.ndarray, names_plot: list, out_png: Path, title="NOW – Matriz de confusión"):
    """Plot a confusion matrix with labeled axes and save to disk."""
    plt.figure(figsize=(5.6, 4.8), dpi=120)
    im = plt.imshow(cm, interpolation="nearest")
    plt.title(title)
    plt.xlabel("Predicho"); plt.ylabel("Real")
    ticks = np.arange(len(names_plot))
    plt.xticks(ticks, names_plot, rotation=45, ha="right")
...
def find_trainlog_for_now(ckpt_prefix: str) -> Path:
    """Find a likely training log CSV (either NOW or segmentation) near the checkpoint."""
...
def plot_training_curves(train_csv: Path, out_png: Path):
    """Plot some common loss columns if present."""
    try:
...
def measure_speed(model, ds, batches=30):
    """Naive inference-speed probe: average ms/image and FPS across a few batches."""
    import time
    it = iter(ds)
    # warmup
    for _ in range(3):
...
def resolve_ckpt(dir_path: Path):
    """Pick a checkpoint prefix inside a directory using a priority pattern order."""
...
# --------- Replot from cache (no evaluation) ----------
def replot_from_cache(report_dir: Path):
    """
    Recreate figures from existing artifacts without re-running inference:
    - Reads summary.json for recall/IoU and class names.
    - Uses now_confusion.npy (if present) to rebuild the confusion figure with labels.
    """
    summary_path = report_dir/"summary.json"
    if not summary_path.exists():
        print(f"[warn] No hay summary.json en {report_dir}; no se puede replotear."); return

    with open(summary_path, "r") as f:
        summary = json.load(f)

    # Recall / IoU from summary
    seg = summary.get("metrics", {}).get("seg", {})
    ...
    ious = [seg.get("iou_per_class", {}).get("sky"),
            seg.get("iou_per_class", {}).get("thick"),
            seg.get("iou_per_class", {}).get("thin"),
            seg.get("iou_per_class", {}).get("sun")]
    ...
    # Confusion matrix if cached .npy exists
    cm_path = report_dir/"now_confusion.npy"
    if cm_path.exists():
        cm = np.load(cm_path)
        names_all = summary.get("class_names", class_names_all(include_void=True))
        # Rebuild label list excluding VOID if needed and align with cm shape
        try:
            void_idx = int(models.VOID_IDX)
        except Exception:
            void_idx = None
        keep = [i for i in range(len(names_all)) if (void_idx is None or i != void_idx)]
        names_plot = [names_all[i] for i in keep][:cm.shape[0]]  # safety slice if shapes differ
        plot_confusion(cm, names_plot, report_dir/"now_confusion.png")
    else:
        print("[warn] Falta now_confusion.npy; no puedo replotear la matriz de confusión sin reevaluar.")

# --------- Single-model evaluation ----------
def run_eval_now(ckpt_prefix: str, csv_path: Path, report_dir: Path):
    """Evaluate a single NOW checkpoint, compute metrics, cache artifacts, and save a concise report."""
    set_policy_float32()
    ...
    # Confusion matrix: drop VOID (if defined), save raw cm to .npy, then plot with labels
    cm = confmat.copy()
    names_all = class_names_all(include_void=True)
    keep_idx = list(range(NUM_CLASSES))
    try:
        void_idx = int(models.VOID_IDX)
        if 0 <= void_idx < NUM_CLASSES:
            cm = np.delete(cm, void_idx, axis=0)
            cm = np.delete(cm, void_idx, axis=1)
            keep_idx = [i for i in keep_idx if i != void_idx]
    except Exception:
        pass
    names_plot = [names_all[i] for i in keep_idx]

    # Cache the raw confusion for future replots (no re-eval needed next time)
    np.save(report_dir/"now_confusion.npy", cm)
    # Plot with labeled axes (and fixed tight_layout)
    plot_confusion(cm, names_plot, report_dir/"now_confusion.png")
    ...
    summary = {
        ...
        "metrics": { "seg": seg },
        # Store class names to make replot fully self-contained
        "class_names": class_names_all(include_void=True)
    }
...
def main():
    """CLI entry point: evaluate checkpoints or rebuild figures from cache for one/multiple tags."""
    ...
    # New flags for caching/replot
    ap.add_argument("--replot-only", action="store_true",
                    help="Do NOT evaluate models; re-generate figures from summary.json/now_confusion.npy if present.")
    ap.add_argument("--skip-eval-if-exists", dest="skip_eval_if_exists", action="store_true",
                    help="If a report already exists for a tag, skip evaluation and only re-generate figures.")
    args = ap.parse_args()
    ...
    # --- Build item list lazily so we only resolve checkpoints when needed ---
    # Each item: dict(tag=..., ckpt=Optional[str], dir=Optional[Path])
    items = []
    if args.ckpt:
        ...
        for ckpt, tag in zip(ckpts, tags):
            # In replot-only we don't need ckpt at all, but keeping it doesn't hurt
            items.append(dict(tag=tag, ckpt=ckpt, dir=None))
    else:
        ...
        for d, tag in zip(dirs, tags):
            ...
            items.append(dict(tag=tag, ckpt=None, dir=base))
    ...
    for item in items:
        ...
        # If only replotting, just rebuild figures from cache and continue
        if args.replot_only:
            print(f"[info] Replot desde caché: {out_dir}")
            replot_from_cache(out_dir)
            # Try to load summary for later tables (if present)
            summary = None
            if (out_dir/"summary.json").exists():
                with open(out_dir/"summary.json","r") as f:
                    summary = json.load(f)
            if summary is None:
                # Build a minimal stub so later code doesn't crash
                summary = dict(metrics=dict(seg={}), speed={}, ckpt_prefix="")
            reports.append((tag, summary, out_dir))
            if outer: outer.update(1)
            continue

        # If skip-eval-if-exists and report is already there, replot and skip evaluation
        if args.skip_eval_if_exists and (out_dir/"summary.json").exists():
            print(f"[info] Ya existe summary para {tag}. Replot sin reevaluar.")
            replot_from_cache(out_dir)
            with open(out_dir/"summary.json","r") as f:
                summary = json.load(f)
            reports.append((tag, summary, out_dir))
        ...
        # Otherwise we need to evaluate: ensure we have a ckpt prefix
        ckpt_prefix = item["ckpt"]
        if ckpt_prefix is None:
            if item["dir"] is None:
                raise SystemExit(f"No se pudo resolver checkpoint para {tag}.")
            ckpt_prefix = resolve_ckpt(item["dir"])
    ...
    if len(reports) >= 2:
        ...
        for tag, summary, _ in reports:
            ...
            rows.append(dict(
                ...
                ms_per_img   = summary.get("speed", {}).get("ms_per_img", np.nan),
                FPS          = summary.get("speed", {}).get("fps", np.nan),
